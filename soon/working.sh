...
"Voir les informations du paquetage"
dpkg -s $REPLY ;;
...
Copier un fichier)
cp $REPLY (exclude number);;

Se connecter en ssh vers une IP)

"Résoudre le DNS")
host $REPLY (exclude number);;

"Changer les permissions utilisateurs du fichiers")
sudo chmod u+rwx ;;

"Changer les permissions groupes du fichiers rwx")
sudo chmod g+rwx ;;

"Accorder les permissions à tous")
sudo chmod 777 ;;
...
"Installer Libre Office 6.1")
sudo apt-get install libreoffice6.1 ;;
